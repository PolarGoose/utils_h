...
struct timed_out_exception final : public std::runtime_error {
  explicit timed_out_exception(char const* const msg)
      : std::runtime_error(msg) {}
};

struct interrupted_exception final : public std::runtime_error {
  explicit interrupted_exception(char const* const msg)
      : std::runtime_error(msg) {}
};
...
public:
  void wait_for(
    ...
    if (timeout) {
      m_cond_var.wait_for(lock, *timeout, [&] {
        return predicate() || _is_interrupt_requested;
      });
    } else {
