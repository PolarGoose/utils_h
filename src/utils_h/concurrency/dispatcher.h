...
public:
  explicit dispatcher(std::function<void(const std::exception_ptr&)>
                          unhandled_exception_handler)
      : _unhandled_exception_handler{std::move(unhandled_exception_handler)},
        _task_processing_thread{&dispatcher::tasks_processing, this} {}

  [[nodiscard]] bool is_dispatcher_thread() const {
    return std::this_thread::get_id() == _task_processing_thread.get_id();
  }
